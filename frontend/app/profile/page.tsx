                          ...
                          <div key={b.id} className="border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                            ...
                            {expandedBooking === b.id && (
                              <div className="border-t border-gray-100 bg-gray-50 p-5">
                                ...
                                {/* Generate Next Bill — only for active monthly bookings */}
                                {(b.status === 'CONFIRMED' || b.status === 'PENDING') && b.next_bill_date && (
                                  <button
                                    onClick={async () => {
                                      try {
                                        const token = localStorage.getItem('token');
                                        const res = await fetch(`${API_URL}/api/bookings/billing/generate`, {
                                          method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                                          body: JSON.stringify({ booking_id: b.id })
                                        });
                                        const data = await res.json();
                                        if (data.success) {
                                          alert(`Bill #${data.data.billing_cycle} generated!\n${data.data.message}\nTotal: ${data.data.total_amount}`);
                                          fetchBookings(bookingFilter);
                                        } else { alert(data.message || 'Failed to generate bill'); }
                                      } catch { alert('Network error'); }
                                    }}
                                    className="inline-flex items-center gap-1.5 mt-3 px-4 py-2 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-sm font-medium hover:bg-emerald-100 transition"
                                  >

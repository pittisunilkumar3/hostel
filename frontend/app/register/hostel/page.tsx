...
export default function HostelRegister() {
  ...
  useEffect(() => {
    (async () => {
      try {        
        ...
        const fd = await (await fetch("http://localhost:3001/api/settings/facebook-status")).json();
        if (fd.success && fd.data.active) setFacebookActive(true);
        const ad = await (await fetch("http://localhost:3001/api/settings/apple-status")).json();
        if (ad.success && ad.data.active) setAppleActive(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const fullAddress = [form.hostelAddress, form.hostelCity, form.hostelState, form.hostelZip].filter(Boolean).join(", ");
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    if (!otpPhone || otpPhone.length < 10) { setError("Enter a valid phone number"); return; }
    setOtpLoading(true);
    try {
  ...
  const ic = "w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all text-sm";

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex items-center justify-center p-4">
      {anySocial && googleClientId && <script src="https://accounts.google.com/gsi/client" async defer />}
      <div className="w-full max-w-[460px]">
        <Link href={ownerLoginUrl} className="inline-flex items-center text-indigo-300 text-sm mb-5 hover:underline">
          <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          Back to Login
        </Link>

        <div className="bg-white/[0.07] backdrop-blur-xl rounded-3xl p-7 shadow-2xl shadow-indigo-900/20 border border-white/10">
          <div className="text-center mb-6">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl mb-3 shadow-lg shadow-indigo-600/30">
            ...
            <h1 className="text-xl font-bold text-white">Register Your Hostel</h1>
            <p className="text-indigo-300/60 text-xs mt-1">List your hostel and start managing bookings</p>
          </div>
          ...
          {loginSetup.manualLogin && <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            {/* Password */}
            <div className="bg-white/[0.03] rounded-xl p-3.5 space-y-3 border border-white/5">
              <p className="text-[10px] text-indigo-400 font-semibold uppercase tracking-widest">🔒 Set Password</p>
              <div className="relative">
                <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500">
            ...
            {/* Hostel Info */}
            <div className="bg-white/[0.03] rounded-xl p-3.5 space-y-3 border border-white/5">
              <p className="text-[10px] text-indigo-400 font-semibold uppercase tracking-widest">🏠 Hostel Details</p>
              <div className="relative">
                <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                </div>
                <input type="text" value={form.hostelName} onChange={(e) => update("hostelName", e.target.value)} required placeholder="Hostel Name *" className={ic + " pl-10"} />
              </div>
              <div className="relative">
                <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                </div>
                <select value={form.hostelType} onChange={(e) => update("hostelType", e.target.value)} className={ic + " pl-10 appearance-none"}>
                  <option value="" disabled className="bg-slate-900">Select Hostel Type</option>
                  <option value="BOYS" className="bg-slate-900">Boys Hostel</option>
                  <option value="GIRLS" className="bg-slate-900">Girls Hostel</option>
                  <option value="CO_ED" className="bg-slate-900">Co-Ed Hostel</option>
                </select>
              </div>
            ...
            {/* Hostel Address */}
            <div className="bg-white/[0.03] rounded-xl p-3.5 space-y-3 border border-white/5">
              <p className="text-[10px] text-indigo-400 font-semibold uppercase tracking-widest">📍 Hostel Address</p>
              <textarea value={form.hostelAddress} onChange={(e) => update("hostelAddress", e.target.value)} required placeholder="Street Address *" rows={2} className={ic + " resize-none"} />
              <div className="grid grid-cols-2 gap-3">
                <input type="text" value={form.hostelCity} onChange={(e) => update("hostelCity", e.target.value)} required placeholder="City *" className={ic} />
                <input type="text" value={form.hostelState} onChange={(e) => update("hostelState", e.target.value)} placeholder="State" className={ic} />
              </div>
              <input type="text" value={form.hostelZip} onChange={(e) => update("hostelZip", e.target.value)} placeholder="Zip / Pin Code" className={ic} />
            </div>

            {/* ID Proof */}
            <div className="bg-white/[0.03] rounded-xl p-3.5 space-y-3 border border-white/5">
              <p className="text-[10px] text-indigo-400 font-semibold uppercase tracking-widest">🪪 Identity Verification</p>
              <div className="relative">
                <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" /></svg>
                </div>
                <input type="text" value={form.idProof} onChange={(e) => update("idProof", e.target.value)} placeholder="ID Proof Number (Aadhar/PAN) *" className={ic + " pl-10"} />
              </div>
            </div>
                  ...
                  {!otpSent ? (
                    <form onSubmit={handleSendOTP} className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input type="tel" value={otpPhone} onChange={(e) => setOtpPhone(e.target.value)} required placeholder="+91 9876543210" className="w-full pl-10 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 text-sm transition-all" />
                      </div>
                      <button type="submit" disabled={otpLoading} className="px-5 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl font-semibold text-sm shadow-lg shadow-indigo-600/20 disabled:opacity-50 transition-all shrink-0">
                        {otpLoading ? "..." : "Send OTP"}
                      </button>
                    </form>
                  ) : (
                    <form onSubmit={handleVerifyOTP} className="space-y-2.5">
                      <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-2.5 text-center">
                        <p className="text-indigo-300 text-xs">
                          OTP sent to <span className="font-bold">{otpPhone}</span>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} required placeholder="• • • • • •" maxLength={6}
                          className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-center text-xl tracking-[0.5em] placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all" />
                        <button type="submit" disabled={otpLoading} className="px-5 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl font-semibold text-sm shadow-lg shadow-indigo-600/20 disabled:opacity-50 transition-all shrink-0">
                          {otpLoading ? "..." : "Verify"}
                        </button>
                      </div>
        ...
        <p className="text-center text-gray-400 text-sm mt-5">
          ...
          <Link href={ownerLoginUrl} className="text-indigo-400 font-semibold hover:underline">Sign In</Link>
        </p>
      </div>
    </div>
  );

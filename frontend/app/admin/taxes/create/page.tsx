  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setMessage({ type: "error", text: res.message || "Failed to create tax" });
      }
    } catch {
    ...
    >
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create a new tax rate for hostel bookings</p>
        </div>
        <Link
          href="/admin/taxes"
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-2xl p-5">
        <h3 className="font-bold text-blue-900 mb-3 flex items-center gap-2">
          ...
          Common Tax Rates (India)
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            { name: "CGST", rate: "9", desc: "Central GST" },
            { name: "SGST", rate: "9", desc: "State GST" },
            { name: "IGST", rate: "18", desc: "Integrated GST" },
            { name: "GST 5%", rate: "5", desc: "For rooms {symbol}1,000-7500" },
            { name: "GST 12%", rate: "12", desc: "For rooms {symbol}7,500+" },
            { name: "GST 18%", rate: "18", desc: "For premium services" },
          ].map((tax) => (
            <button
              key={tax.name}
              onClick={() => {
                setName(tax.name);
                setRate(tax.rate);
                setDescription(tax.desc);
              }}
              className="p-3 bg-white border border-blue-200 rounded-xl text-left hover:bg-blue-50 transition-all"
            >
              <span className="font-medium text-blue-900">{tax.name}</span>
              <span className="text-blue-600 text-sm ml-2">{tax.rate}%</span>
              <p className="text-xs text-blue-500 mt-0.5">{tax.desc}</p>
            </button>
          ))}
        </div>
      </div>

...
export default function AdminSettings() {
  ...
  // Profile fields (mirrors reference settings blade)
  ...
  // Section nav (mirrors reference navbarVerticalNavMenu)
  ...
  // ── Save profile (mirrors reference admin.settings route) ──
  const saveProfile = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const u = getCurrentUser();
        if (u) {
          u.name = `${firstName.trim()} ${lastName.trim()}`.trim();
          u.email = email.trim();
          u.phone = phone.trim();
          if (res.data?.image) u.image = res.data.image;
  ...
  // ── Change password (mirrors reference admin.settings-password route) ──
  ...
  return (
    <DashboardShell
      ...
      hoverBg="bg-white/10"
    >
      {/* Page Header — mirrors reference settings blade page-header */}
      ...
      {/* Content — mirrors reference settings blade row layout */}
      <div className="flex flex-col lg:flex-row gap-6">

        {/* ── Left: Nav menu (mirrors reference navbarVerticalNavMenu) ── */}
        <div className="w-full lg:w-64 shrink-0">
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden lg:sticky lg:top-24">
            <div className="p-4">
              <h3 className="text-sm font-bold text-gray-900 mb-3">{t("nav_menu", "Nav Menu")}</h3>
        ...
        <div className="flex-1 space-y-5">
          ...
          {/* Profile cover + avatar card — mirrors reference settings blade first card */}
            ...
            <div className="h-28 bg-gradient-to-r from-purple-500 to-indigo-600 relative">
              <div className="absolute -bottom-10 left-6">
                <label htmlFor="avatarUploader" className="cursor-pointer group">
                  ...
                  <div className="absolute bottom-0 left-14 w-7 h-7 bg-white rounded-full shadow-md flex items-center justify-center group-hover:bg-purple-50 transition-colors">
          ...
          {/* ── Basic Information — mirrors reference settings blade generalDiv ── */}
          {section === "general" && (
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-50 flex items-center gap-3">
                ...
                <h3 className="text-base font-bold text-gray-900">{t("basic_information", "Basic Information")}</h3>
              </div>
              <div className="p-6 space-y-5">
                {/* Full Name — mirrors reference f_name + l_name inputs */}
                ...
                {/* Phone — mirrors reference phone input */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">
                    {t("phone", "Phone")} <span className="text-gray-400 text-xs">({t("optional", "optional")})</span>
                  </label>
                  <input
                    type="tel"
                    value={phone}
                ...
                {/* Email — mirrors reference email input */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">
                ...
                {/* Save button — mirrors reference save button */}
          ...
          {/* ── Change Password — mirrors reference settings blade passwordDiv ── */}
          {section === "password" && (
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-50 flex items-center gap-3">
                <svg className="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <h3 className="text-base font-bold text-gray-900">{t("change_your_password", "Change Your Password")}</h3>
              </div>
              <div className="p-6 space-y-5">
                {/* New Password — mirrors reference password input */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  ...
                  <p className="text-[11px] text-gray-400 mt-1.5">
                    {t("password_hint", "Must contain at least one number, one uppercase, one lowercase letter, and at least 8 characters.")}
                  </p>
                </div>

                {/* Confirm Password — mirrors reference confirm_password input */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">

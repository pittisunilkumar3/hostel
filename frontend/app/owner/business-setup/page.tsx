            ...
            <div className="bg-gray-50 px-6 py-4 border-b border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Add any additional information about your hostel.</p>
                </div>
            ...
            <div className="p-6 space-y-3">
              {Object.entries(customFields).map(([key, value]) => (
                <div key={key} className="flex items-center gap-3">
                  <input type="text" value={key.replace(/_/g, " ").replace(/field \d+/, "")} placeholder="Field name"
                    onChange={(e) => {
                      const newKey = e.target.value.replace(/\s+/g, "_").toLowerCase();
                      const newFields = { ...customFields };
                      delete newFields[key];
                      newFields[newKey || `field_${Date.now()}`] = value;

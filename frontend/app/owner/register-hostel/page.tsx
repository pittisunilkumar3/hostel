...
export default function RegisterHostelPage() {
  ...
  const handleCoverChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setError("Cover must be less than 5MB");
      return;
    }
    setCoverFile(file);
  ...
  // Success popup — mirrors reference registration complete
  if (success) {
    return (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-emerald-300/70 text-xs leading-relaxed">
                    Thank you for submitting your hostel registration! Our team is reviewing your application. We will get back to you as soon as possible.
                  </p>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        {/* Step Header — mirrors reference step__header */}
        <div className="mb-8">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                    step === s.num
                      ...
                      : step > s.num
                      ? "bg-emerald-600/30 text-emerald-300 cursor-pointer"
                      : "bg-white/5 text-gray-500"
                  }`}
                >
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                    step > s.num ? "bg-emerald-500 text-white" : step === s.num ? "bg-white text-emerald-700" : "bg-white/10 text-gray-500"
                  }`}>
                    {step > s.num ? "✓" : s.num}
                  </span>
          ...
          <div className="space-y-5">
            <div className="bg-white/[0.07] backdrop-blur-xl rounded-2xl p-6 border border-white/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input type="text" value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="e.g. Sunrise Hostel" className={ic} />
                </div>
                <div className="md:col-span-2">
                  <label className={labelCls}>Address <span className="text-red-400">*</span></label>
                  <input type="text" value={form.address} onChange={(e) => update("address", e.target.value)} placeholder="Full address" className={ic} />
                </div>
                <div>
                  ...
                  <select value={form.zone_id} onChange={(e) => update("zone_id", e.target.value)} className={ic + " bg-white/5"}>
                    ...
                    {zones.map((z) => (
                      <option key={z.id} value={z.id} className="bg-slate-800">{z.name}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className={labelCls}>Phone <span className="text-red-400">*</span></label>
                  <input type="tel" value={form.phone} onChange={(e) => update("phone", e.target.value)} placeholder="+91 9876543210" className={ic} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" step="any" value={form.latitude} onChange={(e) => update("latitude", e.target.value)} placeholder="e.g. 28.6139" className={ic} />
                </div>
                <div>
                  <label className={labelCls}>Longitude</label>
                  <input type="number" step="any" value={form.longitude} onChange={(e) => update("longitude", e.target.value)} placeholder="e.g. 77.2090" className={ic} />
                </div>
                <div className="md:col-span-2">
                  <label className={labelCls}>Description</label>
                  <textarea value={form.description} onChange={(e) => update("description", e.target.value)} rows={3} placeholder="Tell us about your hostel..." className={ic + " resize-none"} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/[0.07] backdrop-blur-xl rounded-2xl p-6 border border-white/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Logo */}
                <div>
                  <label className={labelCls}>Hostel Logo (1:1 ratio)</label>
                  <input ref={logoRef} type="file" accept="image/*" onChange={handleLogoChange} className="hidden" />
                  <div
                    onClick={() => logoRef.current?.click()}
                    className="cursor-pointer border-2 border-dashed border-white/20 hover:border-emerald-500/50 rounded-xl p-6 text-center transition-all"
                  >
                    {logoPreview ? (
                ...
                {/* Cover */}
                <div>
                  <label className={labelCls}>Cover Photo (3:1 ratio)</label>
            ...
            <div className="bg-white/[0.07] backdrop-blur-xl rounded-2xl p-6 border border-white/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={form.total_rooms} onChange={(e) => update("total_rooms", e.target.value)} placeholder="e.g. 20" className={ic} />
                </div>
                <div>
                  <label className={labelCls}>Total Beds <span className="text-red-400">*</span></label>
                  <input type="number" value={form.total_beds} onChange={(e) => update("total_beds", e.target.value)} placeholder="e.g. 50" className={ic} />
                </div>
                <div>
                  <label className={labelCls}>Minimum Stay (days)</label>
                  <input type="number" value={form.min_stay_days} onChange={(e) => update("min_stay_days", e.target.value)} placeholder="1" className={ic} />
                </div>
                <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="bg-white/[0.07] backdrop-blur-xl rounded-2xl p-6 border border-white/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.owner_f_name} onChange={(e) => update("owner_f_name", e.target.value)} placeholder="First name" className={ic} />
                </div>
                <div>
                  <label className={labelCls}>Last Name</label>
                  <input type="text" value={form.owner_l_name} onChange={(e) => update("owner_l_name", e.target.value)} placeholder="Last name" className={ic} />
                </div>
                <div>
                  <label className={labelCls}>Phone <span className="text-red-400">*</span></label>
                  <input type="tel" value={form.owner_phone} onChange={(e) => update("owner_phone", e.target.value)} placeholder="+91 9876543210" className={ic} />
                </div>
                <div>
                  <label className={labelCls}>Email</label>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-yellow-300/70 text-xs mt-1 leading-relaxed">
                      After submitting your application, our team will review your hostel details. Once approved, you&apos;ll be able to access the owner dashboard to manage your hostel, rooms, and bookings.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-between">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-8 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl font-semibold text-sm shadow-lg shadow-emerald-600/25 disabled:opacity-50 transition-all flex items-center gap-2"
              >
                {loading ? (
                  <>
